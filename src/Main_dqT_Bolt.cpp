...
//        double CosThetaQ=qZ/qAbs;
        ...
        double fp=PhaseSpaceDistribution::fQBolt(pT,yP,EtaX,Xi,Teff,qSupp);
        double fqMp=PhaseSpaceDistribution::fQBolt(qMpT,yqMP,EtaX,Xi,Teff,qSupp);
    ...
    void SampledNdQdy(double QSqr,double qTMin,double qTMax,double TauMin,double TauMax,double EtaQ,double dNchdEta,double Area,double etas,double &dN,double &dNPreEq,double &dNHydro){
        ...
        double EtaMin=-10+EtaQ;
        double EtaMax=10+EtaQ;
		...
		if (NQ > 1){
			...
			Q=QMin;
        	    ...
        	    double dN=0.0; double dNPreEq=0.0; double dNHydro=0.0;
            	    DileptonRates::SampledNd2qTdQdy_v2(Q,qT,TauMin,TauMax,yQ,dNchdEta,Area,dN,dNPreEq,dNHydro,EtaOverS);
